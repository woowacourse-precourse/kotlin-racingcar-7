...
- 터미널에서 Mac 또는 Linux 사용자의 경우 `./gradlew clean test` 명령을 실행하고, Windows 사용자의
  경우 `gradlew.bat clean test` 또는 `.\gradlew.bat clean test` 명령을 실행할 때 모든 테스트가 아래와 같이 통과하는지 확인한다.
...
- [x] 자동차는 이름을 가진다. String
- [x] 자동차는 위치를 가진다. Int
    - [x] 처음 위치는 0에서 시작한다.
- [x] 자동차는 매번 랜덤의 숫자를 가진다. Int

#### 자동차의 기능

- [x] 전진 : 위치를 1 증가시킨다.
    - [x] 전진 조건 : (0-9)의 무작위 값이 4 이상일 때
...
- [x] 마지막 플레이 후 가장 큰 위치를 가진 자동차
- [x] 반환 : 자동차의 이름(들) String

#### 주행 현황

- [x] 자동차 이름을 담은 Map을 받는다.
- [x] 전진 횟수 만큼 하이픈(-)을 출력한다.
-

#### 예외 처리

- 플레이 횟수 입력 예외 처리
    - [x] 사용자가 양수가 아닌 정수(0, 음수)를 입력할 때 예외를 발생시킨다.
    - [x] 사용자가 숫자가 아닌 문자를 입력할 때 예외를 발생시킨다. (소수점 포함)
    - [x] 횟수 입력이 없을 때 예외를 발생시킨다.

- 경주할 자동차 이름 입력 예외 처리
    - [x] 쉼표를 제외한 다른 기호(특수문자)는 예외를 발생시킨다.
    - [x] 자동차 이름이 6자 이상이면 예외를 발생시킨다.
    - [x] 자동차 이름 입력이 없으면 예외를 발생시킨다.

#### 나만의 기준?

- 동일한 이름은 하나의 자동차로 인식한다.
    - 동일한 이름이면 우승자 구분이 어렵기 때문이다.
- 경주할 자동차가 한 대일 때도 경주는 진행한다.
    - 한 대일 때 우승자를 지정하는 데에 문제가 없으며, 전진 조건이 무작위이므로 다른 실행 결과를 보여줄 수 있기 때문이다.
- [x] 자동차 이름에 숫자를 넣을 수 있다. 숫자만으로 이루어져 있어도 무방하다.
    - 그렇게 기준을 세운 이유는 구분자인 콤마(,)에 영향을 끼치지 않으면서 구분이 가능하기 때문이다.
    - 동일한 이름(지원)이어도 숫자를 적으면(지원1, 지원2) 우승자가 누군지인지 구분이 가능하기 때문이다.
    - [x] 숫자만으로 이루어져 있어도 동일한 숫자를 넣지 않으면 구분할 수 있으므로 가능
...
1. `feat`: 함수에 새로운 기능을 추가
2. `fix`: 버그를 수정
3. `refactory`: 코드 개선, 기능 변경이 없을 때, 함수의 이름 변경
4. `style`: 공백 제거, 코드 정렬, 괄호 위치 수정 등
5. `docs`: README, API 문서 등의 변경 사항
6. `test`: 테스트 추가, 테스트 수정
7. `chore`: 코드 변경이 아닌 변경사항(의존성 업데이트, 프로젝트 설정 변경 등)
...
- [x] 문맥을 분리하기 위해 공백을 사용하였는가? 공백을 남용하진 않았는가?
- [x] 의미 없는 주석을 사용하진 않았는가? 주석보다 네이밍에 의도를 담았는가?
- [x] 배열보다는 컬렉션을 사용하였는가?